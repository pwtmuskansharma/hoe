    ...
    const fetchMembers = async () => {
      try {
        ...
        const sortedCards = [...cardData].sort((a, b) => {
          const aIndex = MEMBER_TYPE_PRIORITY.indexOf(
            a.member_type?.trim() || "",
          );
          const bIndex = MEMBER_TYPE_PRIORITY.indexOf(
            b.member_type?.trim() || "",
          );

          const safeA = aIndex === -1 ? MEMBER_TYPE_PRIORITY.length : aIndex;
          const safeB = bIndex === -1 ? MEMBER_TYPE_PRIORITY.length : bIndex;

          return safeA - safeB; // highest first
        });
